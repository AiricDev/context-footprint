...
impl DocumentationScorer for HeuristicDocScorer {
    fn score(&self, _node_info: &NodeInfo, doc_text: Option<&str>) -> f32 {
        ...
        if doc_lower.contains("args") || doc_lower.contains("parameters") || doc_lower.contains("param") {
            keyword_score += 0.2;
        }
        if doc_lower.contains("raises") || doc_lower.contains("exceptions") || doc_lower.contains("throws") {
            keyword_score += 0.1;
        }
