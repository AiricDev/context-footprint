...
use crate::domain::policy::{SizeFunction, DocumentationScorer, NodeInfo, NodeType};
use crate::domain::node::{Node, NodeCore, FunctionNode, VariableNode, TypeNode, SourceSpan, Visibility, Mutability, VariableKind, TypeKind};
use crate::domain::semantic::{SemanticData, SymbolKind, ReferenceRole, SymbolMetadata};
...
impl GraphBuilder {
    ...
    pub fn build(
        ...
        for document in &semantic_data.documents {
            let source_path = Path::new(&semantic_data.project_root)
            ...
            for definition in &document.definitions {
                ...
                // Compute doc_score
                let doc_text = definition.metadata.documentation.first().map(|s| s.as_str());
            ...
            for reference in &document.references {
                if let (Some(source_idx), Some(target_idx)) = (
                    ...
                    if matches!(edge_kind, EdgeKind::Write) {
                        state_writers.entry(reference.symbol.clone()).or_default().push(source_idx);
                    }
                    ...
                    if matches!(edge_kind, EdgeKind::Call) {
                        callers.entry(reference.symbol.clone()).or_default().push(source_idx);
                    }
        ...
        for callee_symbol in symbols {
            if let Some(callee_idx) = graph.get_node_by_symbol(&callee_symbol) {
                if let Some(caller_indices) = callers.get(&callee_symbol) {
...
fn infer_node_type_from_kind(kind: &SymbolKind) -> NodeType {
    match kind {
        ...
        | SymbolKind::StaticMethod | SymbolKind::AbstractMethod => NodeType::Function,
        SymbolKind::Variable | SymbolKind::Field | SymbolKind::Constant 
        | SymbolKind::Parameter => NodeType::Variable,
        SymbolKind::Class | SymbolKind::Interface | SymbolKind::Struct 
        | SymbolKind::Enum | SymbolKind::TypeAlias | SymbolKind::Trait 
