...
use context_footprint::domain::node::NodeId;
...
fn compute_cf(semantic_data: SemanticData, start_symbol: &str) -> (Arc<context_footprint::domain::graph::ContextGraph>, context_footprint::domain::solver::CfResult) {
...
fn assert_reachable(graph: &context_footprint::domain::graph::ContextGraph, result: &context_footprint::domain::solver::CfResult, symbol: &str) {
    let idx = graph.get_node_by_symbol(symbol).expect(&format!("Symbol {} not in graph", symbol));
    let id = graph.node(idx).core().id;
    assert!(result.reachable_set.contains(&id), "Symbol {} should be reachable", symbol);
}

fn assert_not_reachable(graph: &context_footprint::domain::graph::ContextGraph, result: &context_footprint::domain::solver::CfResult, symbol: &str) {
    let idx = graph.get_node_by_symbol(symbol).expect(&format!("Symbol {} not in graph", symbol));
    let id = graph.node(idx).core().id;
    assert!(!result.reachable_set.contains(&id), "Symbol {} should NOT be reachable", symbol);
}
    ...
    let documents = vec![
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                type_def(sym_strategy_interface, "IStrategy", vec!["Strategy Interface".into()], TypeKind::Interface, true),
    ...
    let docs_factory = vec![
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                // Interface
                type_def(sym_strategy_interface, "IStrategy", vec!["Doc".into()], TypeKind::Interface, true),
...
fn test_observer_pattern_boundary() {
    ...
    let sym_observer_interface = "IObserver";
        ...
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                function_def(
                    ...
                    vec![ParameterInfo { name: "event".into(), param_type: Some("Event".into()), ..Default::default() }],
                    Some("void".into())
                ),
        ...
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                function_def(
                    ...
                    vec![ParameterInfo { name: "req".into(), param_type: Some("Request".into()), ..Default::default() }],
                    Some("Response".into())
                ),
        ...
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                function_def(
                    ...
                    vec![ParameterInfo { name: "data".into(), param_type: Some("Data".into()), ..Default::default() }],
                    Some("Result".into())
                ),
        ...
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                function_def(
                    ...
                    "do_stuff", 
                    vec![], // No docs
                    vec![ParameterInfo { name: "x".into(), param_type: None, ..Default::default() }], // Untyped
                    None // No return type
                ),
    ...
    let documents = vec![
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                function_def(sym_singleton_get, "get_instance", vec![], vec![], Some("Singleton".into())),
                
                // Global Variable (Mutable)
                variable_def(sym_global_instance, "_instance", vec![], Some("Singleton".into()), Mutability::Mutable),
...
fn test_law_of_demeter_violation_penalty() {
    ...
    assert!(result_good.total_context_size < result_bad.total_context_size, 
        ...
        DocumentSemantics {
            ...
            definitions: vec![
                ...
                function_def(
                    ...
                    vec![ParameterInfo { name: "msg".into(), param_type: Some("str".into()), ..Default::default() }],
                    Some("void".into())
                ),
            ],
            references: vec![
